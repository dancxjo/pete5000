...
export class AudioProcessingService {
    static async decodeWebm(webmData: ArrayBuffer): Promise<AudioBuffer> {
        const startedAt = Date.now();
        const tempDir = Deno.makeTempDirSync();
        const tempWebmPath = join(tempDir, `${uuidv4()}.webm`);
        const tempWavPath = join(tempDir, `${uuidv4()}.wav`);
    ...
    static async decode(audioData: Uint8Array): Promise<AudioBuffer> {
    ...
    static async toWav(audioBuffer: AudioBuffer): Promise<Uint8Array> {
